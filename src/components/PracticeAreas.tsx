...
export default function PracticeAreas({ showFullListing = false }: PracticeAreasProps) {
            ...
            return (
              <div key={category.id} className="mb-14 sm:mb-16 last:mb-0">
                <div className="flex items-center gap-4 mb-6 sm:mb-8">
                  <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl icon-gold flex items-center justify-center shrink-0">
                  ...
                  <div className="hidden sm:flex items-center gap-3">
                    <span className="rounded-full bg-gradient-to-r from-gold/15 to-gold/5 border border-gold/15 px-4 py-1.5 text-xs font-bold text-gold-dark">
                      ...
                      <div className="p-6 sm:p-8 relative">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex items-center gap-3.5">
                            <span className="h-12 w-12 rounded-xl icon-gold flex items-center justify-center font-heading font-bold text-black text-lg shrink-0">
                              {area.letter}
                            </span>
                            <div>
                          ...
                          <Link
                            ...
                            className="shrink-0 h-9 w-9 rounded-full bg-gold/8 group-hover:bg-gold/20 flex items-center justify-center transition-all duration-300"
                          >
                            <ArrowRight className="h-4 w-4 text-gold/50 group-hover:text-gold-dark transition-all duration-300" strokeWidth={2} />
                          </Link>
                        </div>
                        ...
                        <div className="space-y-2">
                          {area.services.map((service) => (
                            <div key={service} className="flex items-start gap-2.5">
                              <div className="mt-1 h-4 w-4 rounded-full bg-gradient-to-br from-gold/20 to-gold/5 flex items-center justify-center shrink-0">
                        ...
                        <Link
                          href={`/practice-areas/${area.slug}`}
                          className="mt-6 inline-flex items-center gap-2 rounded-lg border border-gold-dark/20 px-4 py-2.5 text-xs font-semibold uppercase tracking-widest text-gold-dark hover:bg-gold/10 hover:border-gold/30 transition-all duration-300"
                        >
                          View Details
                ...
                <div className="mt-14 sm:mt-16 section-separator" />
              </div>
            );
          })}
        </div>
  ...
  /* Homepage mode — Tabs + Practice area grid */
  return (
    <section id="practice-areas" className="bg-cream cream-pattern py-20 sm:py-28 lg:py-36">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            return (
              <button
                ...
                className={`shrink-0 flex items-center gap-2 rounded-full px-4 sm:px-5 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold transition-all duration-300 border ${
                  isActive
                    ? "bg-gradient-to-r from-gold to-gold-light text-black border-gold/30 shadow-lg shadow-gold/25 gold-glow-sm"
                    : "bg-white/80 text-gray-600 border-gray-200/80 hover:border-gold/25 hover:text-gold-dark hover:shadow-md"
                }`}
              >
        ...
        {/* Practice areas in active category */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {activeAreas.map((area) => (
            ...
            >
              ...
              <ChevronRight className="h-4 w-4 text-gold/40 group-hover:text-gold-dark shrink-0 transition-all duration-300 group-hover:translate-x-0.5" strokeWidth={2} />
            </Link>
          ))}
        </div>
