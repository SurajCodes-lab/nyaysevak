      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <ScrollReveal>
          <div className="lg:flex lg:items-end lg:justify-between lg:gap-8 mb-10 sm:mb-14">
            ...
            {/* Tab toggle — right side on desktop */}
            <div className="mt-6 lg:mt-0 flex lg:flex-col items-start gap-3">
              <div className="bg-cream-dark rounded-full p-1.5 inline-flex shadow-inner shadow-black/[0.04]">
                <button
          ...
          >
            {services.map((service) => {
              ...
              return (
                <div
                  key={service.slug}
                  className="snap-child shrink-0 w-[82vw] sm:w-[65vw] md:w-[45vw] rounded-2xl glass-cream overflow-hidden"
                >
                  <div className="h-1 bg-gradient-to-r from-gold/20 via-gold/40 to-gold/20" />
                  <div className="p-5 sm:p-6 flex flex-col h-full">
                    <div className="h-11 w-11 sm:h-12 sm:w-12 rounded-xl bg-gradient-to-br from-gold/15 to-gold/5 border border-gold/10 flex items-center justify-center mb-4">
                      {IconComponent && <IconComponent className="h-5 w-5 sm:h-6 sm:w-6 text-gold-dark" strokeWidth={1.5} />}
                    </div>
        ...
        <div className="hidden lg:grid grid-cols-[1fr_1fr] gap-6 xl:gap-8">
          ...
          <div className="row-span-3 rounded-2xl overflow-hidden glass-cream !border-gold/15 flex flex-col">
            <div className="h-1.5 bg-gradient-to-r from-gold via-gold-light to-gold" />
            <div className="p-8 xl:p-10 flex flex-col flex-1">
              <div className="h-16 w-16 xl:h-18 xl:w-18 rounded-2xl bg-gradient-to-br from-gold to-gold-dark flex items-center justify-center mb-6 shadow-lg shadow-gold/20">
                {FeaturedIcon && <FeaturedIcon className="h-8 w-8 xl:h-9 xl:w-9 text-black" strokeWidth={1.5} />}
              </div>

              <h3 className="text-2xl xl:text-3xl font-heading font-bold text-gray-900 mb-3">
                {featured.title}
              </h3>
              ...
              <ul className="space-y-3 flex-1 mb-8">
                {featured.bullets.map((b) => (
                  <li key={b} className="flex items-start gap-3 text-sm text-gray-600">
                    <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-gradient-to-br from-gold to-gold-dark" />
                    {b}
                  </li>
                ))}
              </ul>
          ...
          {/* Remaining services — compact list on right */}
          {rest.map((service) => {
            const IconComponent = lucideIconMap[service.lucideIcon];
            return (
              <Link
                key={service.slug}
                href={`/services/${service.slug}`}
                className="group flex items-start gap-5 rounded-2xl glass-cream p-6 xl:p-7 transition-all duration-400"
              >
                <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-gold/15 to-gold/5 border border-gold/10 flex items-center justify-center shrink-0 group-hover:from-gold/25 group-hover:to-gold/10 group-hover:border-gold/20 transition-all duration-300">
                ...
                <ArrowRight className="h-4 w-4 text-gold/30 group-hover:text-gold-dark shrink-0 mt-1 transition-all duration-300 group-hover:translate-x-1" strokeWidth={2} />
              </Link>
            );
          })}
        </div>
        ...
        {/* Audience pills */}
        <ScrollReveal>
          <div className="mt-12 sm:mt-16 pt-6 border-t border-gold/10">
            <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-3">
              ...
              {audiences.map((a) => (
                <span
                  key={a.label}
                  className="rounded-full bg-white/60 backdrop-blur-sm border border-white/50 px-4 py-2 text-xs sm:text-sm text-gray-600 font-medium shadow-sm"
                >
                  {a.label}
                </span>
              ))}
